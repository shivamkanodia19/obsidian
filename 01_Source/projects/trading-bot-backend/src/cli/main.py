...
def doctor():
    ...
    if risk_yaml.exists():
        try:
            ...
            if limits.allow_live_trading:
                issues.append("DANGER: allow_live_trading is TRUE — live orders could be placed")
            else:
    ...
    if settings.kalshi_api_key_id and settings.kalshi_private_key_path:
        ...
        if key_path.exists():
            ...
            issues.append(f"Kalshi private key not found at: {settings.kalshi_private_key_path}")
    else:
