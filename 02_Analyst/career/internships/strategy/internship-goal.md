---
title: Internship Goal & Role Targets
project: internships
strategic: false
status: stable
origin_dump: "[[01_Source/internships/Internship goal.md]]"
last_synced_dump: "[[01_Source/internships/Internship goal.md]]"
last_updated: 2026-04-15
tags: [internship, positioning, roles, strategy]
---

# Internship Goal & Role Targets (Summer 2026)

**Core Goal:** Secure technical operations/AI operations internship role leveraging local network + formal applications.

## Target Roles

- AI Operations
- Machine Learning
- Data Science
- Product Management
- Consulting
- Venture Capital

**Positioning:** Technical Operations Hire; builder/executor, not learner (see [[strategy/email-optimization-strategy]] for detailed approach)

## Dual-Channel Outreach Strategy

### Channel 1: Local Company Outreach (Primary)
- **Method:** Email + phone calls to DFW companies
- **Target:** Non-tech companies with operational pain points in automation/efficiency
- **Status:** Wave 3 (Flower Mound Top 5) ready to send; Wave 4 (50–70 DFW companies) in execution
- **Resources:** See [[expansion-playbook]] for 6-phase company discovery workflow

### Channel 2: Formal Applications (Secondary)
Submitted/Active:
- Innovation WorkSpaces AI Operations (Contact: Valerie Gibson)
- Owens Corning externship
- ESL software/data science
- Zachry Leadership Program (TAMU, deadline June 2026)

## Target Mode

**Primary:** Local DFW company outreach (maximize odds of getting offer)  
**Secondary:** Formal applications + broader opportunities

Reasoning: Direct outreach to 50-70 local companies provides broader exposure than formal apps alone.

## Resume

Resume attached in `/01_Source/internships/` folder.

## Sourced From

[[01_Source/internships/Internship goal.md]] (updated 2026-04-15)
3. Track responses + follow-ups

**Medium-term (Weeks 3–6):**
1. Expand to 50–70 DFW companies across 6 industries
2. Monitor formal application status
3. Iterate email frameworks based on Wave 3 response data

---

## Related Files

- [[expansion-playbook]] — Company discovery + research workflow for scaling
- [[industry-frameworks]] — 6 industry-specific email templates + psychology
- [[outreach/wave-3/]] — Wave 3 top 5 emails ready to send
- [[outreach/wave-4/]] — Wave 4 multi-industry expansion in progress
