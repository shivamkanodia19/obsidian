---
name: audit
description: Vault structural cleaner. Detects and auto-fixes orphaned folders, missing indexes, and duplicates. Surfaces broken wikilinks, naming violations, and index-governance drift.
user-invocable: true
---

# /audit

Vault structural cleaner. Runs automatically after the vault save phase of `/save-audit` or standalone to maintain vault organization.
...
1. Scan all folders in `/02_Analyst/`, `/03_References/`, `/04_Archive/`, `/05_Outputs/`
...
5. Distinguish generated output artifacts from human navigation folders before surfacing empty-folder or index-maintenance noise
...
**Check 3: Contradiction & Canonical-State Validation**
1. Detect competing `current` notes and source-of-truth claims
2. Detect sibling alias folders like `dairy farms/` vs `dairy-farms/`
3. Detect duplicate note stems that may represent split truth, while respecting the duplicate-stem policy for intentional redirects and scoped artifacts
4. Detect duplicate `## New Files` headings in `_index.md` files

**Check 4: Task-Context Coverage**
1. For any `_index.md` with `agent_context: true`, verify the frontmatter still includes:
   - `project`
   - `scope`
   - `status`
   - `current_focus`
   ...
   - `prompt_context`
   - `definition_of_done`
2. Surface missing fields in the contradiction audit so prompt-critical routing drift is visible
3. Surface prompt-context drift when those links point to missing or historical notes

Result: Your vault stays clean and navigable. Orphaned structures are removed. Broken links are visible. Missing documentation is auto-created. Hub pages stay useful for fresh agents.
...
Scan `/02_Analyst/`, `/03_References/`, `/04_Archive/`, and `/05_Outputs/` recursively:
...
### 1.5. Check root guidance and hub-page quality

Review the vault entrypoints and top hubs:
- `INDEX.md`
- `CLAUDE.md`
...
For each, check:
- Is it still a fast orientation page rather than a running log?
- Is `last_updated` reasonably fresh if frontmatter exists?
- Does it route cleanly to the next layer down?
- Does it avoid brittle wikilink patterns such as `[[/...]]` and, when a clearer option exists, `[[../...]]`?

If a hub page has turned into a long history log, move that history into a dedicated activity or run-log note and slim the hub.

### 2. Create missing `_index.md` files

For every active folder in `/02_Analyst/`, `/03_References/`, or `/05_Outputs/` **without** `_index.md`:
...
- **Related:** [[02_Analyst/path/to/related-note]]
```
...
- extracted image-only exam/archive folders or placeholder attachment folders that are clearly archival rather than navigational
...
Also surface brittle link style separately:
- rooted wiki links like `[[/...]]`
- unnecessary traversal links like `[[../...]]` when a stable vault-root-relative link would be clearer

### 6. Surface naming violations

Scan **folder names only** (not files) in `/02_Analyst/` and `/03_References/`:

**Rule:** Folder names should prefer lowercase with hyphens when creating new structure, but legacy canonical names do not need to be flagged if renaming them would create more churn than value.
...
- Count open entries (`awaiting_review`, `pending_validation`)
- Count closed entries (`resolved`, `archived`)
- Treat legacy `supported_by_example` entries as `resolved`
- Surface count in report: "Open conflicts: N"

(Do not modify conflict content; only count and report.)

### 8.5. Surface contradiction clusters

Write `.vault-contradictions.md` with:
- competing `current` or source-of-truth note clusters
- sibling alias-folder collisions
- duplicate note stems split into `intentional_redirect`, `scoped_duplicate`, and `review_candidate`
- duplicate `## New Files` headings
- missing task-context fields on `_index.md` files that declare `agent_context: true`

These are surfaced for routing cleanup. Do not auto-delete notes just because the names collide.
...
5a. **Archive noise stays low** — Treat extracted image/archive-only folders as low-priority unless they break active navigation
...
7a. **Prefer hub quality over exhaustiveness** — `_index.md` files should orient quickly, not mirror every historical note
...
**When `/audit` runs (standalone or from the audit phase of `/save-audit`):**
...
10. Step 8.5: Surface contradiction clusters
11. Step 9: Report output

**No blocking points.** All steps complete; report summarizes what was done + what needs user review.

---

## Integration with /save-audit

The vault-facing `/save-audit` flow should call `/audit` immediately after durable synthesis and index updates:
...
- Any contradiction clusters created → write them to `.vault-contradictions.md`
- Include audit report in the final save-audit summary

Append audit report to the save-audit completion message under "Vault Audit".
```

**Result:** Every `/save-audit` run can finish with `/audit` → the user gets both synthesis and cleanup in one command.
...
On next `/save-audit`, `/audit` runs again → fewer issues.
...
## For /save-audit Integration

When modifying a vault save contract, add an audit step after durable synthesis is complete:
...
**Append to report:** Include audit summary under "Vault Audit" in the save-audit output.

If audit finds breaking issues (deleted files, consolidated duplicates), flag them prominently so user knows what changed.
```

Then update the save summary to include a "Vault Audit" section that gets filled by this audit call.
