...
status: current   # current | active | snapshot | historical | draft | superseded | archived
...
1. Classify the change first:
   - replacement = old note should stop being current
   - augmentation = old note still stands, new note adds detail
   - pending_validation = evidence is promising but not strong enough to replace the old note yet
2. Update Analyst anyway (non-blocking)
3. If this is a replacement, downgrade the losing note to `snapshot`, `historical`, or `superseded` instead of leaving two `current` notes in the same scope
4. Set `conflict_detected: true` in frontmatter only when the disagreement is still open
5. Log to `.vault-conflicts` with:
   ```
   [YYYY-MM-DD HH:MM] Conflict in [[02_Analyst/...]]
   Old: [what Analyst said]
   New: [[01_Source/...]] says [what Source says]
   Status: awaiting_review | pending_validation
   ```
6. User sees this in `/resume` and can decide

### 7. Maintain `/02_Analyst/_index.md`

Ownership note: root-derived fields such as `active_projects`, `open_conflicts`, `unreviewed_conflicts`, and `latest_audit_notes` should be treated as `save-audit` outputs. `/save` should prioritize the nearest project or domain `_index.md`, then let `save-audit` refresh the root-level derived state.
...
Use explicit routing sections such as `Current Canonical Notes`, `Historical Notes`, or `Drafts and Queues` when multiple notes on one topic could be confused.

### 7.5. Maintain project-local task context

For the nearest project or topic `_index.md`, prefer project-local prompt context over a separate task file when the work is contained to one area.

If the index declares `agent_context: true`, update these frontmatter fields during save:
- `current_focus`
- `active_tasks`
- `prompt_context`
- `definition_of_done`
- `blocked_by` when that constraint matters
- `surface_in_root: true` only if this is a primary hub that belongs on `02_Analyst/_index.md`

Create a standalone task card only when the task is cross-project, high-stakes, reusable, or needs a reviewer-loop trail.
...
## Contradiction Normalization Rules

- One scope gets one current note.
- If new Source replaces old state, downgrade the older note to `snapshot`, `historical`, or `superseded`.
- Use only `awaiting_review`, `pending_validation`, `resolved`, or `archived` in `.vault-conflicts`.
- When a folder has multiple plausible notes, update its `_index.md` to separate current notes from historical notes and drafts.
