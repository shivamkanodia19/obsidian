...
- Runtime setup is usually one-time per session unless the kernel resets. Do not acquire a starter `tab` just to inspect capabilities or open other tabs.
- After a kernel reset, stale handle, or lost `tab` binding, recover previously claimed agent tabs with `browser.tabs.list()` and `browser.tabs.get(tab.id)`.
- To attach to an already-open visible tab in the in-app browser, call `browser.user.openTabs()`, choose the matching returned tab, then call `browser.user.claimTab(tab)`. Do not pass `openTabs()` ids to `browser.tabs.get(...)`.
- `browser.user.openTabs()` is not the full tab list for the current Browser Use run. It lists user-visible tabs that the current Browser Use run is not already controlling. Tabs already claimed or opened by the current Browser Use run stay on `browser.tabs.list()`.
- When the task is about *all currently open or visible tabs* in the in-app browser, inspect both surfaces:
  - `browser.tabs.list()` for tabs already owned by the current session.
  - `browser.user.openTabs()` for released or not-yet-claimed visible tabs.
- A tab kept with `browser.tabs.finalize({ keep: [{ tab, status: "deliverable" }] })` should move from `browser.tabs.list()` to `browser.user.openTabs()` after finalization. Do not assume `openTabs()` alone is exhaustive while the session still owns tabs.
...
- On the first browser cell in a session, initialize the runtime. Acquire `tab` only when the next action will use that tab. Never write `tab = ...` before `tab` exists.
...
If the next action needs a single tab, choose the acquisition path that matches the task:

Use `browser.tabs.selected()` only when continuing a tab that the current Browser Use run is already controlling:
...
When the page you need is already open as a user-visible tab, claim it instead of opening a duplicate:

```js
const openTabs = await browser.user.openTabs();
const targetTab = openTabs.find((candidate) =>
  (candidate.url ?? "").includes("example.com")
);
globalThis.tab = targetTab
  ? await browser.user.claimTab(targetTab)
  : await browser.tabs.new();
```

When the task is to inspect, compare, or close every visible in-app browser tab, enumerate both the current-session tabs and the unclaimed or released tabs:
...
When no existing tab matters and you want to start fresh, create one:
...
For tasks involving multiple tabs, create descriptive variables for the real tabs you will navigate instead of creating a generic starter `tab`. If `browser.tabs.new()` succeeds but the following navigation fails, close that tab before continuing.
