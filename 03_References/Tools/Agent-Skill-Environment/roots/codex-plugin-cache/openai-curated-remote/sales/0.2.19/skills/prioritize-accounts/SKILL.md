...
This is a rep-facing, account-first workflow. It prioritizes accounts before contacts, branches clearly across `net_new`, `expansion`, or `mixed`, suppresses accounts already in motion, and returns a small set of executable Suggested Focus rows. Its primary output is a connector-grounded account action view that can be opened in Codex when connected source records or useful source links are available. It plans and drafts only: it does not execute outreach, create records, update CRM, or send messages unless the user explicitly asks for that in a separate step.
...
Confirm whether the default account universe, ranking basis, and motion goal are sufficient for the request before searching connectors, reading sources, or drafting an answer. For ambiguous account-prioritization prompts, ask at most three high-impact, high-uncertainty questions only when the answers would materially change the account set, ranking basis, source of truth, or safe next action.

Do not treat connector/source availability as user intent or as a replacement for the requested account scope. If live CRM or another default source is available but the user has not specified a scope, use the defaults in `Default Intake Behavior` and state assumptions in `Scope` instead of blocking. If the default or assumed source returns no owned open pipeline, stop before broadening and ask which adjacent account universe to use next.

When using assumptions, label the result as assumption-based in `Scope`, including the assumed account set, ranking basis, source of truth, and motion goal. Suppress any extra skill-owned final continuation when the clarification questions are already the next step.
