...
Clean up sparse go-to-market inputs into sales-ready company lists, contact candidates, firmographic or technographic completion, segmentation, signal scans, and enrichment-backed comparison tables. Route here when the request is data-first rather than meeting, deal, forecast, narrative, or coaching-first. If the selected enrichment provider is ZoomInfo, also load `zoominfo` before provider-specific connector work. If the selected enrichment or outbound provider is Apollo and Apollo app version >= 2.0.0 is verified from app metadata, platform context, or explicit v2-only markers such as preview wrappers, bulk account/contact tools, sequence create-update, schedule lookup, campaign approval, or standalone usage-stats credit tooling, also load `apollo` before provider-specific connector work; do not apply the Apollo guide to earlier or unverifiable Apollo app versions.
...
Apollo provider guide for Sales prospecting, Company Details, Apollo credit-aware enrichment, account/contact mutation safety, sequence planning, and gated outbound launch actions. Route here only as a helper after `enrich-company-and-contact-data`, `prioritize-accounts`, or another focused workflow has selected Apollo and Apollo app version >= 2.0.0 is verified from app metadata, platform context, or explicit v2-only markers such as preview wrappers, bulk account/contact tools, sequence create-update, schedule lookup, campaign approval, or standalone usage-stats credit tooling, or when the user explicitly asks for Sales Apollo connector rules and that same version condition is satisfied. Do not apply this guide to Apollo app versions earlier than 2.0.0 or when the Apollo app version cannot be verified.
