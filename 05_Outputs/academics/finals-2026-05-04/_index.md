---
title: Finals 2026-05-04 Outputs
description: Final-exam study assets generated during the May 4 sprint
last_updated: 2026-05-06
---

# Finals 2026-05-04 Outputs

## Core Notes

- [[Spring 2026 Final Cheat Sheet]]
- [[Spring 2026 Final Cheat Sheet - Condensed]]
- [[LinkedList-FRQ-Guide]]
- [[LinkedList-FRQ-Guide-Condensed]]

## Printable Variants

- LinkedList FRQ readable and condensed DOCX/PDF variants
- 2D vector FRQ cheat sheet markdown, print HTML, and PDF

## Source Packets

- [[Final Exam Sample.pdf]]
- [[Final Exam Review - Spring 2026 (2).pdf]]

## Build Helpers

- `build_2dvector_cheatsheet_pdf.py`
- `build_condensed_cheatsheet_pdf.py`
- `cheatsheet-template.html`
- `cheatsheet-print.css`
- `2dvector-print.css`
